...
const CardsComponent = ({ ArrayOfCardsImgs, cover, themeName }) => {
  ...
  const accuracy = turns === 0 ? 100 : Math.round(((turns - misses) / turns) * 100);
  ...
  useEffect(() => {
    let interval;
    if (gameStartTime && !gameCompleted && gameStarted) {  // Only run if game has started and isn't completed
  ...
  return (
    <div className="game-container">
      <div className={`card-grid ${blurActivated ? 'game-over-grid' : ''}`}>
      ...
      <div className={`game-footer ${footerActivated ? 'game-over-footer' : ''}`}>
        ...
        <div className="game-footer-buttons">
          <button onClick={shuffleCards} className={`${themeName}-button`}>Reset Game</button>
          <Link to={`/theme/`}><button className={`${themeName}-button`}>Change Theme</button></Link>
        </div>
      </div>
    </div>
  );
